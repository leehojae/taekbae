...
function delayTime2(data, lat, lng) {
  ...
  min[0] = datas[0];
  minTimeUpdate(min, datas, iii+1);
  data.splice(min[0].index,1);
 return datas;
};
...
function delayTime3(data, lat, lng) {
  ...
  params = "version=1&reqCoordType=WGS84GEO"; 
  params += "&startX="+ lng +"&startY=" + lat; 
  params += "&endX="+ data[0].lng+"&endY="+data[0].lat+"&appKey=6a18b782-3680-3c86-98b6-73887bafb90e"; 
  ...
  $.ajax({ 
   ...
   dataType:"JSON", 
   success : function(timeData) {
    datas[0] = new timeTable(data[0].trcno, timeData.features[0].properties.totalTime, 0, data[0].lat, data[0].lng);
   }, 
   error : function(xhr, status, error) { 
    alert(error) 
   } 
...
//  for (var ii = 0, item; item = datas[ii]; ii++){
    ...
    console.log("minloop");
    console.log(datas[i]);
    console.log(min[0].delayTime);
    console.log(datas[i].delayTime);
    if(min[0].delayTime >= datas[i].delayTime){
      min[0] = datas[i];
      console.log(datas[i]);
      console.log("change?");
      console.log(min[0]);
   ...
   $.ajax({
     ...
     async  : false, 
     data   : {trcno     : min[0].trcno, 
               delayTime : min[0].delayTime,  
               stateNum  : count + 1}, 
     dataType : "JSON", 
     success  : function(timeData) {
...
function update(obj) {
  ...
  $.ajax({
    ...
    console.log("2");
    console.log(min);
    console.log(min[0].index);
    console.log("minindex");
    
    min = data.splice(min[0].index,1);
    ...
    for (var iii = 0; iii < loopCount-1; iii++){
     ...
     if (iii != loopCount-2) {
      ...
      delayTime2(data, min[0].lat, min[0].lng);
     ...
     if (iii == loopCount-2) {
      ...
      delayTime3(data, min[0].lat, min[0].lng);
